...
type ColumnType = ColumnDef<RenamedpackageInterface, unknown>;

interface RenamedpackageListPageProps {
...
export function RenamedpackageListPage(props: RenamedpackageListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RenamedpackageInterface>>(
    () => `/renamedpackages?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RenamedpackageInterface) => {
    if (hasAccess('renamedpackage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/renamedpackages/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/renamedpackages/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="renamedpackage" />
        </Flex>
