...
import { getExperiences, deleteExperienceById } from 'apiSdk/experiences';
import { ExperienceInterface } from 'interfaces/experience';

type ColumnType = ColumnDef<ExperienceInterface, unknown>;
...
export function ExperienceListPage(props: ExperienceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ExperienceInterface>>(
    () => `/experiences?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ExperienceInterface) => {
    if (hasAccess('experience', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/experiences/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/experiences/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('experience', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/experiences/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('experience', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="experience" />
        </Flex>

        {hasAccess('experience', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/experiences/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
